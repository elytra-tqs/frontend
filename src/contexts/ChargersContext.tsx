...
interface ChargersContextType {
    ...
    addChargerToStation: (stationId: number, chargerData: ChargerFormData) => Promise<void>;
}
...
export const ChargersProvider = ({ children }: { children: ReactNode }) => {
    ...
    const getChargerAvailability = async (chargerId: number): Promise<ChargerStatus> => {
        try {
            const response = await api.get<ChargerStatus>(`/chargers/${chargerId}/availability`);
    ...
    const updateChargerAvailability = async (chargerId: number, status: ChargerStatus): Promise<void> => {
        try {
            await api.put(`/chargers/${chargerId}/availability`, JSON.stringify(status), {
    ...
    const fetchChargersByAvailability = async (status: ChargerStatus): Promise<void> => {
        try {
            ...
            const response = await api.get<Charger[]>(`/chargers/availability/${status}`);
    ...
    const addChargerToStation = async (stationId: number, chargerData: ChargerFormData): Promise<void> => {
        try {
            setLoading(true);
            setError(null);
            const response = await api.post<Charger>(`/stations/${stationId}/chargers`, chargerData);
